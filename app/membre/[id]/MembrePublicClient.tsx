...
export default function MembrePublicClientPage({ memberId }: { memberId: string }) {
      ...
      try {
        ...
        if (!cancelled) {
          if (auth?.user) {
            ...
            const { data: favs } = await supabase.from("favoris").select("annonce_id").eq("user_id", auth.user.id);
        ...
        const { data, error } = await supabase.from("annonces").select("*").eq("user_id", memberId).order("id", { ascending: false });
  ...
  const confianceBadge = evalSummary && evalSummary.count >= 3 && evalSummary.avg >= 4;
    ...
    <div className="membreRoot" style={{ fontFamily: "sans-serif" }}>
      ...
      <main style={{ maxWidth: "1100px", margin: "0 auto", padding: "0 20px" }}>
        <div style={{ padding: "26px 0 18px" }}>
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
              ...
              <div>
                <div style={{ fontSize: "22px", fontWeight: 600, marginBottom: "2px" }}>{loading ? "Chargement..." : memberNom}</div>
                {!loading && (
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
                  ...
                  {!loading && <span style={{ fontSize: "12px", color: "#666" }}>{annonces.length} annonce{annonces.length > 1 ? "s" : ""}</span>}
                </div>
              </div>
            </div>
        ...
        {!loading && evalDerniers.length > 0 && (
          ...
          >
            <h2 style={{ margin: "0 0 14px", fontSize: "16px", fontWeight: 600, color: "#0F6E56" }}>Derniers avis</h2>
              ...
              >
                ...
                <div className="annonceBody" style={{ padding: "14px", textAlign: "center" }}>
                  <div style={{ display: "flex", justifyContent: "center", gap: "4px", flexWrap: "wrap", marginBottom: "6px" }}>
                    <span className="annonceCategory" style={{ fontSize: "12px", padding: "3px 10px", borderRadius: "20px", background: "#E1F5EE", color: "#0F6E56" }}>
                      {annonce.categorie}
                    </span>
